...
use crate::{Size, VecDeku, VecLayout};

impl<T: Clone + std::cmp::PartialOrd> VecDeku<T> {
        ...
        R: no_std_io::Read + no_std_io::Seek,
    {
        ...
        <Vec<T>>::from_reader_with_ctx(reader, (limit, inner_ctx)).map(VecDeku)
    }
...
where
    V: Clone + DekuReader<'a, Endian> + std::cmp::PartialOrd,
{
...
where
    Ctx: Copy,
    V: Clone + DekuReader<'a, (Endian, Ctx)> + std::cmp::PartialOrd,
{
    ///
...
where
    V: Clone + Default + DekuWriter<Endian> + std::cmp::PartialOrd,
{
...
where
    Ctx: Copy,
    V: Clone + Default + DekuWriter<(Endian, Ctx)> + std::cmp::PartialOrd,
{
    fn to_writer<W: no_std_io::Write + no_std_io::Seek>(
        &self,
        writer: &mut Writer<W>,
